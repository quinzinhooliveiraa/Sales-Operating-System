...
// 1. Add missing state for "Manage Pipeline" and task creation
...
// 2. Hook up "Gerenciar Pipeline" button
content = content.replace(
  /<Button variant="outline" className="h-8 text-xs gap-1.5">[\s\S]*?<Settings2 className="w-3.5 h-3.5" \/>[\s\S]*?Gerenciar Pipeline[\s\S]*?<\/Button>/,
...
// 3. Add Task Creation buttons in Lead Sheet
content = content.replace(
  /<div className="flex gap-2">\n\s*<Button className="flex-1 bg-primary text-primary-foreground gap-2"><Phone className="w-4 h-4"\/> Ligar<\/Button>\n\s*<Button variant="outline" className="flex-1 gap-2"><Mail className="w-4 h-4"\/> Email<\/Button>\n\s*<\/div>/,
  `<div className="flex gap-2">
...
// 4. Add "Manage Pipeline" Dialog and "Create Task" Dialog
const newModals = `
...
console.log('CRM features added');
